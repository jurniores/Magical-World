...
using Omni.Core;
using UnityEngine;
public class MovimentBotServer : BaseMoveBots
{
    ...
    public override void OnTick(ITickInfo data)
    {
        ...
        if (distPlayerAndServer > 3 && !lagClientReposition)
        {
            ...
            using DataBuffer buffer = NetworkManager.Pool.Rent();
            buffer.FastWrite((HalfVector3)transform.position);
